...
export type WalletEntryKind = 'activity_reward' | 'takeover_cost' | 'deal_transfer' | 'penalty' | 'correction';
...
export interface PresenceEvent { id: string; memberId: string; action: PresenceAction; at: string }
...
export interface AppSnapshot { members: Member[]; meals: MealPlan[]; presence: PresenceEvent[]; tasks: Task[]; wallet: WalletEntry[]; takeovers: Takeover[]; deals: Deal[]; }

export interface RpcResult { [key: string]: unknown }
export interface HouseholdAdapter {
  mode: 'demo' | 'supabase';
  snapshot(): Promise<AppSnapshot>;
  complete_task(input: { taskId: string; idempotencyKey: string; performedByMemberId: string | null }): Promise<RpcResult>;
  ...
  recordPresence(action: PresenceAction): Promise<void>;
}
